...
class dt_onoff extends data_type {
    ...
    public function form_html($value, $formerror, $name= false){
    	...
    	return '<div style="padding-top:4px;"></div><input type="checkbox" '.(isset($fieldOpts['modify_readonly'])?' disabled="true" ':'').' name="'.$field['field_index'].'" '.$checked.' value="1" id="'.$field['field_index'].'"><label for="'.$field['field_index'].'" class="icon"></label>';
    }
}
?>