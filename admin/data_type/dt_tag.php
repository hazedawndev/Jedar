        ...
        if (is_array($_POST[$field['field_index']])) {
            foreach ($_POST[$field['field_index']] as $tag) {
                if ($existtags[$tag] == false) {
                    ...
                    try {
                        if($field['item_type'] == 'game_items'){
                            $type = 'game_items';
                        } else {
                            list($type, $item_type_id) = explode('_', $field['item_type']);
                        }
                        if(($type == 'game' ||  $type == 'forum' ||  $type == 'items') && !$item_type_id){
                            $item_type_id = $_GET[$field['item_id']];
                        }
                        $stmt = $this->mysql->prepare('INSERT INTO gameffy_tag_level (tag_id, item_type, item_type_id, status, date_added, top)
                        SELECT :tag_id, :item_type, :item_type_id, :status, :date_added , 10
                        WHERE NOT EXISTS ( SELECT * FROM gameffy_tag_level WHERE tag_id = :tag_id2 AND item_type = :item_type2 AND item_type_id = :item_type_id2) LIMIT 1;');
