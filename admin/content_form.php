...
<div class="row">
    <div class="<?php echo $curCfg['contentwidth_class']?$curCfg['contentwidth_class']:'col-md-12'; ?> col-xs-12">
        <div class="x_panel">
            ...
            <div class="x_content">
                <form class="form-horizontal form-label-left form_<?php echo $_GET['t']; ?>" action="?<?php echo get_link('form'); ?>" method="POST" enctype="multipart/form-data" id="enter_form">
                    ...
                    foreach ($curCfg['fields'] as $field) { ?>
                        ...
                        <?php $fieldOpts = fieldOpt($field['field_options']); ?>
                        <?php if ((!isset($_GET['stage']) || !isset($fieldOpts[$_GET['stage']])) && (!isset($curCfg['table_type']) || $curCfg['table_type'] != 'setting') && !(isset($fieldOpts['create']) && $_GET['stage'] == 'duplicate') && !(isset($fieldOpts['modify_show']) && $_GET['stage'] == 'modify')) {
                            continue;
                        } ?>
                        <?php if($field['field_type'] == 'hidden'){ $data_type->{'dt_'.$field['field_type']}->config($field); echo $data_type->{'dt_'.$field['field_type']}->form_html($_POST, isset($formerror)?$formerror:array()); continue;} ?>
                        <?php if($field['field_type'] == 'html'){echo $field['field_name']; continue;} ?>
                        <div class="form-group<?php if (isset($formerror) && isset($formerror[$field['field_index']]) && $formerror[$field['field_index']]) { ?> has-error<?php } ?> <?php echo $field['field_index'].' '.$curCfg['table_index']; ?>">
                            ...
                            <div class="col-md-9 col-sm-9 col-xs-12">
                                ...
                                <?php } else if ((isset($_GET['stage']) && ($_GET['stage'] == 'create' || $_GET['stage'] == 'modify' || $_GET['stage'] == 'duplicate')) || $curCfg['table_type'] == 'setting') { ?>
                                    <?php echo $data_type->{'dt_'.$field['field_type']}->form_html($_POST, isset($formerror)?$formerror:array()); ?>
                                <?php } ?>
                                <!-- create modify  -->
                    ...
                    <?php if ((isset($_GET['stage']) && ($_GET['stage'] == 'create' || $_GET['stage'] == 'modify' || $_GET['stage'] == 'details' || $_GET['stage'] == 'duplicate')) || $curCfg['table_type'] == 'setting') { ?>
                        ...
                        <div class="form-group">
                            <div class="col-md-9 col-sm-9 col-xs-12 col-md-offset-3">
                                <?php if (!isset($curCfg['table_type']) || $curCfg['table_type'] != 'setting') { ?>
