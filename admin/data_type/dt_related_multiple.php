    ...
    public function filter_html($value){
        ...
        if (isset($rtCfg['table_type']) && $rtCfg['table_type'] == 'tree') {
            ...
            if($field['related_extra_key'] && isset($field['related_extra_value']) && $field['related_extra_value'] != ''){
                ...
                $mainList = $this->mysql->getList($parentCfgs['table_name'], array(), '*', $parentCfgs['table_order_field'], $parentCfgs['table_order_default_direction']);
            }
            $dataByID = array();
            ...
            if($field['related_extra_key'] && isset($field['related_extra_value']) && $field['related_extra_value'] != ''){
                ...
                $selectList = $this->mysql->getList($rtCfg['table_name'], array(), '*', $rtCfg['table_order_field'], $rtCfg['table_order_default_direction']);
            }
        }
        $fieldType = '';
        ...
        foreach ($selectList as $info) {
            ...
            if (preg_match('/\,/is', $field['related_name'])) {
                ...
                $info[$field['related_name']] = join(' ', $names);
            }
            $html .= 'value="'.$info[$field['related_key']].'">'.$parent_name.'/'.$info[$field['related_name']].'</option>';
        }
        $html .= '</select>';
    ...
    public function form_html($value, $formerror, $name= false){
        ...
        if (isset($rtCfg['table_type']) && $rtCfg['table_type'] == 'sublist' && $rtCfg['parent_table']) {
            ...
            function getTree($parentID){
                global $dataByID, $rtCfg, $subList, $field, $rtOpts, $level, $_POST, $tbCfgs, $relate_checked;
                foreach (getListByParent2($subList, $parentID) as $key => $row) {
                    ...
                    $output .= '<label for="'.$field['related_table'] . '_' . $row[$rtCfg['table_primarykey']].'">'.$parent_name.$label.'</label></p>';
            ...
            if($rtCfg['table_index'] == 'admin_zconfigtable_fields'){
                ...
                $subList = $this->mysql->getList($rtCfg['table_name'], array(), '*', $rtCfg['table_order_field'], $rtCfg['table_order_default_direction']);
            }
            $dataByID = array();
            foreach ($subList as $data) {
                $dataByID[$data[$rtCfg['table_primarykey']]] = $data;
    ...
    private function process_data(){
        ...
        if (isset($rtCfg['table_type']) && $rtCfg['table_type'] == 'sublist' && $rtCfg['parent_table']) {
            ...
            if($field['related_extra_key'] && isset($field['related_extra_value'])){
                ...
                $mainList = $this->mysql->getList($tbCfgs[$rtCfg['parent_table']]['table_name'], array(), '*', $tbCfgs[$rtCfg['parent_table']]['table_order_field'], $tbCfgs[$rtCfg['parent_table']]['table_order_default_direction']);
            }
            foreach ($mainList as $minfo) {
