...
if (isset($_POST['form_stage']) && $_POST['form_stage'] == 'form_submit') {
	...
	if (!count($formerror) && $_POST['cnt_btns_btn'] == 'btn_build_tableconfig') {
        ...
        if($_POST['language'] && ($_POST['images_language'] || $_POST['videos_language'] || $_POST['files_language'] || $_POST['text_language'])){
            $lang_defindex = false;
            $tabs = array();
            foreach (unserialize($_POST['language']) as $lang) {
            ...
            foreach ($langs as $key => $lang) {
                ...
                if($_POST['images_language']){
                    foreach (unserialize($_POST['images_language']) as $image) {
                        ...
                        $fields[] = array(
                            ...
                            'extra_opt' => serialize(array($imgopts))
                        );
                    }
                }
                if($_POST['videos_language']){
                    foreach (unserialize($_POST['videos_none_language']) as $video) {
                        ...
                        $fields[] = array(
                            ...
                            'extra_opt' => serialize(array($vidopts))
                        );
                    }
                }
                if($_POST['files_language']){
                    foreach (unserialize($_POST['files_none_language']) as $file) {
                        ...
                        $fields[] = array(
                            ...
                            'extra_opt' => serialize(array($fleopts))
                        );
                    }
                    
                }
                if($_POST['text_language']){
                    foreach (unserialize($_POST['text_language']) as $text){
        ...
        foreach (@unserialize($_POST['images_none_language']) as $image) {
            $opts = str_getcsv($image[2], ',', "'");
            $imgopts = array();
            $imgopts[0] = $opts[0];
            $imgopts[1] = $opts[1];
        ...
        foreach (@unserialize($_POST['text_none_language']) as $text){
            $opts = str_getcsv($text[2], ',', "'");
            $fields[] = array(
                'field_index' => $text[0],
                'field_name' => $text[1],
        ...
        // print_r(unserialize('a:1:{i:0;a:7:{i:0;s:6:"resize";i:1;s:4:"2000";i:2;s:4:"2000";i:3;s:0:"";i:4;s:2:"10";i:5;s:9:".jpg,.png";i:6;s:1:"0";}}'));
        // exit;
        ...
        $newData = array(
            ...
            'table_option' => 'allow_list,allow_create,allow_delete,allow_details,allow_modify',
            'status' => 1,
            'date_added' => $nowTime,
