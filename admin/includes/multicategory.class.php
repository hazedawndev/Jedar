...
class multiCat {
    ...
    function getTree_dev($allData, $parentID)
    {
        global $curCfg, $text_system, $curCfg;
        foreach ($this->getListByParent($allData, $parentID) as $key => $row) {
            $catid = $row[$this->indexKeyName];
            ...
            if (count($this->getListByParent($allData, $catid))) {
                if ($numRow == true) {
                    $pix = '<img border="0" src="' . $this->imageUrl . 'tree7.gif">';
                    $top = 'background="' . $this->imageUrl . 'tree5.gif"';
                } else {
                    $pix = '<img border="0"  src="' . $this->imageUrl . 'tree2.gif">';
                    $top = '';
                }
                $subhtml = "<tr><td $top>&nbsp;</td><td colspan=\"2\">" . $this->getTree_dev($allData, $catid) . "</td></tr>";
                $folder = '<img alt="" src="' . $this->imageUrl . 'admin_folder.gif" border="0">';
            } else {
                if ($numRow == true) {
                    $pix = '<img src="' . $this->imageUrl . 'tree7.gif">';
            ...
            $show_field = '';
            $btn_update = ':title:<input size="60" type="text" name="title_tc_' . $row[$curCfg['table_primarykey']] . '" value="' . htmlspecialchars($row['title_tc']) . '">';
            $btn_update .= '&nbsp;&nbsp;&nbsp;tc:<input size="60" type="text" name="link_tc_' . $row[$curCfg['table_primarykey']] . '" value="' . htmlspecialchars($row['link_tc']) . '">';
			...
			" . htmlspecialchars($row[$this->indexfieldName]) . "&nbsp;$show_field &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			$btn_ord $btn_cre $btn_mod $btn_det $btn_del $btn_update
			</td>
			</tr>
			$subhtml
			</table>";
