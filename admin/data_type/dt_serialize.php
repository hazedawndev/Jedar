...
class dt_serialize extends data_type {
    ...
    public function form_html($value, $formerror, $name= false){
        ...
        $output .= '$(\'#serialize_'.$field['serialize_key'].'\').append(\''.$this->serialize_html($field['fields_cfg'], array(), $field['serialize_key']).'\');';
        $output .= '}</script>';
        ...
        if ((isset($_POST[$field['field_index']]) && $_POST[$field['field_index']]) || ($default_value && $_GET['stage'] == 'create')) {
            ...
            $output .= $this->serialize_html($field['fields_cfg'], array(), $field['serialize_key']);
        }
        $output .= '</div>';
