...
if(isset($_GET['sub_stage'])){
    ...
    if($_POST['cnt_btns_btn'] == 'create_db' && $curCfg['table_name'] == 'admin_zconfigtable'){
        if($mysql->tableExists($_POST['table_name'])){
            ...
            if(!isset($message_popup)){
                ...
                $fields = $mysql->getList('admin_zconfigtable_fields', array('admin_zconfigtable_id' => $_GET['admin_zconfigtable_id']));
                if($fields){
                    foreach ($fields as $field) {
                        $fieldOpts = fieldOpt(explode(',', $field['field_options']));
                        if (isset($fieldOpts['skipsql']) && $fieldOpts['skipsql'] && $created_fields[$field['field_index']]) {
                            continue;
                        }
                ...
                if(isset($_POST['table_create_field']) && $_POST['table_create_field']){
                    $create_fields[] = '`'.$_POST['table_create_field'].'` datetime  NULL';
                }
                if(isset($_POST['table_modify_field']) && $_POST['table_modify_field']){
                    $create_fields[] = '`'.$_POST['table_modify_field'].'` datetime  NULL';
                }
                if(!$created_fields['admin_added']){
                    $create_fields[] = '`admin_added` int(11) NULL';
                }
                if(!$created_fields['admin_modified']){
    ...
    if (isset($formerror) && !count($formerror)) {
        if ($_GET['stage'] == 'create' || $_GET['stage'] == 'duplicate') {
            ...
            if($_GET['stage'] == 'create' && !isset($adminPris[getPriActID($t, 'allow_'.$_GET['stage'])]) && $login->getAdminID() != 1){
                echo 'permission denied';
                exit;
            }
            ...
            if ($curCfg['table_order_field'] && $curCfg['table_order_type'] == 'order') {
                if($_GET['order_from']){
                    ...
                    foreach (explode(',', $curCfg['table_order_field']) as $order_field) {
                        ...
                        $lastrecord = $mysql->getData($curCfg['table_name'], $parent_query, '*', array($order_field), 'DESC');
            ...
            if($_GET['stage'] == 'duplicate' && $_POST['db_name']){
                $mysql->change_db($_POST['db_name'], $_POST['db_name'].'_');
            }
