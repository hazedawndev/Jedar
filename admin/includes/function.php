...
function get_php_file($type, $cfg){
    ...
    if($type == 'list' || $type == 'backlist' || $type == 'sublist' || $type == 'shortcut'){
        ...
        } else if(isset($cfg['table_type']) && $cfg['table_type'] && file_exists($cfg['table_type'] . '.php')){
        ...
        if(isset($cfg['modify_url']) && isset($cfg[$type.'_url']) && $cfg[$type.'_url']){
            return 'project_file.php?file='.str_replace('.php', '', $cfg[$type.'_url']);
        } else if(isset($cfg['modify_url']) && $cfg['modify_url'] && $type == 'details') {
            return 'project_file.php?file='.str_replace('.php', '', $cfg['modify_url']);
        } else {
            return 'content.php?';
        }
    }
...
function get_link($type='', $cfg=null, $key='') {
    ...
    if($type == 'details' || $type == 'create' || $type == 'modify' || $type == 'duplicate'){
        ...
        return get_php_file($type, $cfg).'&t='.$cfg['table_index'].'&m='.$key.(isset($cfg['extra_urlpar'])?$cfg['extra_urlpar']:'').'&cc=1&ca=1'."&tt=$type".'&sl_id='.$_GET['sl_id'];
...
function is_relate_checked($tfield) {
    ...
    $subList = $mysql->getList($reCfgs['table_name'], array());
    $checked = false;
...
function getImageUrl($id) {
    ...
    $folder = get_folder($admin_imagefolder, $id, $imageData['date_added']);
...
function getRelateValue($field, $id) {
    ...
    foreach (explode(',', $field['related_name']) as $fieldname) {
        foreach ($rtCfg['fields'] as $rekey => $reField){
            if($reField['field_index'] != $fieldname){continue;}
            if($reField['field_type'] == 'image'){
                ...
                $value .= getRelateValue($reField, $relatedData[$fieldname]).' ';
