...
class data_type {
	...
	public function config_model(){
		$allow_create = false;
		return array(array(), $allow_create);
	}
	...
	public function form_html($value, $formerror, $name= false){
		...
		$output = '<input autocomplete="off" type="'.$type.'" class="'.$error.'form-control '.$datetime.' field_'.$field['field_index'].'" '.$maxlength.' jf="'.$field['field_index'].'"
           placeholder="'.strip_tags($field['field_remark']?$field['field_remark']:get_systext($field['field_name'])).'" style="width: 100%;" '.(isset($fieldOpts['modify_readonly'])?' readonly="true" ':'').'
