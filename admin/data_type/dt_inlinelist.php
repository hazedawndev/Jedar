...
class dt_inlinelist extends data_type {
    ...
    public function list_value($values){
        ...
        $field = $this->config();
        $data = @unserialize(isset($values[$field['field_index']])?$values[$field['field_index']]:$default_value);
        $data = $data?$data:array(array());
        $output = '<table border="1" cellpadding="3">';
        $output .= $this->serialize_list_value($field['fields_cfg'], array(), $field['serialize_key'], true);
    ...
    public function form_html($value, $formerror, $name= false){
        ...
        if ((isset($_POST[$field['field_index']]) && $_POST[$field['field_index']]) || ($default_value && $_GET['stage'] == 'create')) {
            ...
            if(!$field['allow_create']){
                ...
                list($html, $js) = $this->serialize_html($field['fields_cfg'], array(), $field['serialize_key'], true);
                $output .= $html;
                foreach ($data as $sdata) {
            ...
            list($html, $js) = $this->serialize_html($field['fields_cfg'], array(), $field['serialize_key']);
            $output .= $html;
            $javascript .= $js;
        } else {
            list($html, $js) = $this->serialize_html($field['fields_cfg'], $sdata, $field['serialize_key'], true);
            $output .= $html;
            $javascript .= $js;
        }
        $output .= '</div>';
